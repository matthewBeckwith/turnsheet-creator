...
const Turnsheet = () => {
    ...
    useEffect(() => {  // Assign or Create TurnsheetID for Firebase
        console.log("hello from assign or create ID");
        ID ? setTurnsheetID(ID) : 
        setTurnsheetID(uniqueID);
      }, [uniqueID]);

    useEffect(() => {  // Is this an Existing Turnsheet?
        console.log("hello from does this turn exist");
        if(turnsheetID !== null){
            firebase.database().ref(`turnsheets`).on('value', snapshot => {
                snapshot.hasChild(turnsheetID) ? setTurnsheet(snapshot.val()) : 
                setTurnsheet({
                    unit_address:'',
    ...
    useEffect(() => {  // Convert Turnsheet into seperate state variables
                ...
                <div>
                    <Grid container justify="space-between">
                        <Grid item xs={8} container>
                            <Grid item xs={9}>
                                <Typography variant="caption">Address: {unitAddress}</Typography>
                            </Grid>
                            <Grid item xs={3}>
                                <Typography variant="caption">Total: {grandTotal}</Typography>
                            </Grid>
                            <Grid item xs={6}>
                                <Typography variant="caption">Security Deposit: {unitSecurityDeposit}</Typography>
                            </Grid>
                            <Grid item xs={6}>
                                <Typography variant="caption">Owner Balance: {ownerBalance}</Typography>
                            </Grid>
                        </Grid>
                        <Grid item xs={3}>
                    ...
                    {rooms.length > 0 ?
                        <Grid container>
                            {rooms.map((room, index) => <Room key={`room-${index}`} handleGrandTotal={handleGrandTotal} handleDelRoom={handleDelRoom} roomIndex={index} roomObj={room} />)}
                        </Grid>
    ...
    return (
        <Grid item xs={12} md={6} lg={4}>
            <Grid container justify="space-between">
                ...
                <Grid item>
                    <Button size="small" onClick={() => handleDelRoom(roomIndex)}>Remove Room</Button>
                </Grid>
            </Grid>
            {items.length > 0 ?
                <div>
                    {items.map((item, index) => <Item key={`${roomObj.room_name}-item-${index}`} handleDelItem={handleDelItem} itemObj={item} index={index} />)}
                </div> : <Typography variant="caption">No Items</Typography>
            }
        </Grid>
    )
}
...
const Item = ({handleDelItem, itemObj, index}) => {
    return (
        <Grid container justify="space-between">
            <Grid item>
                <Typography variant="caption">{itemObj.item_description} - ${itemObj.item_estimated_total_cost}</Typography>
            </Grid>
            <Grid item>
                <Button size="small" onClick={() => handleDelItem(index)}>Remove Item</Button>
            </Grid>
        </Grid>
    )
}
